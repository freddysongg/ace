...
# Optional SHAP import for feature importance analysis
try:
    import shap
    SHAP_AVAILABLE = True
except ImportError:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data",
        type=str,
        default="data/input_with_sequences_v1.npy",
    )
    parser.add_argument(
        "--year-column",
        type=int,
...
def main():
    ...
    # Ensure only single pollutant selection
    ...
    # Validate geographic region bounds if specified
    if args.test_region:
        try:
    ...
    # Load pre-built sequential data from disk
    print("Loading pre-built sequential data...")
    sequence_dir = Path("data/sequences_lookback7")

    if not sequence_dir.exists():
        raise FileNotFoundError(f"Sequence directory not found: {sequence_dir}")

    # Load sequence arrays and metadata
    X_train_seq = np.load(sequence_dir / "X_train.npy")
    y_train_seq = np.load(sequence_dir / "y_train.npy")
    X_val_seq = np.load(sequence_dir / "X_val.npy")
    y_val_seq = np.load(sequence_dir / "y_val.npy")
    X_test_seq = np.load(sequence_dir / "X_test.npy")
    y_test_seq = np.load(sequence_dir / "y_test.npy")

    with open(sequence_dir / "meta.pkl", "rb") as f:
        meta = pickle.load(f)

    target_scaler = meta["target_scaler"]
    lons_test = meta["lons_test"]
    lats_test = meta["lats_test"]
    lookback = X_train_seq.shape[1]  # Extract lookback period from data shape

    print(f"Loaded data with lookback of {lookback} days.")
    print(
        f"Sequential data shapes - Train: {X_train_seq.shape}, Val: {X_val_seq.shape}, Test: {X_test_seq.shape}"
    )

    # Create compatibility object for legacy code integration  
    class DummySplitter:
        def get_config_dict(self):
            return {"strategy": "pre_built_sequences", "lookback": lookback}
    ...
    data_splitter = DummySplitter()

    # Configure shapefile path for spatial visualization
    shapefile_for_map = "data/cb/cb_2018_us_state_20m.shp"

    # Load column metadata for feature/target identification
    ...
    # Map pollutant names to column name variations
    ...
    # Locate target column indices in the dataset
    try:
        ozone_idx = next(
            i
        ...
        # Preserve original case for display names
    ...
    # Separate feature columns from target columns for SHAP analysis
    ...
    # Configure training parameters for each pollutant type
    all_pollutant_configs = {
        "Ozone": {
            "target_index": 0,
            "use_robust_scaler_targets": False,
            "log_transform": False,
            "epochs": args.epochs,
    ...
    # Select pollutants based on command-line flags
    pollutant_configs = {}
    if args.ozone:
    ...
    with mlflow.start_run(
        ...
        mlflow.log_param("model_type", "cnn_lstm_sequential_lookback")
        mlflow.log_param("data_source", "pre_built_sequences")
        mlflow.log_param("sequence_dir", str(sequence_dir))
        ...
        for pollutant_name, config in pollutant_configs.items():
            ...
            # The data is already preprocessed and in sequences.
            # We just need to select the correct target column for each pollutant.
            target_idx = config["target_index"]
            y_train_single_pollutant = y_train_seq[:, target_idx]
            y_val_single_pollutant = y_val_seq[:, target_idx]
            y_test_single_pollutant = y_test_seq[:, target_idx]

            print(f"Selected target index {target_idx} for {pollutant_name}")
            print(
                f"Sequential data shapes - X_train: {X_train_seq.shape}, y_train: {y_train_single_pollutant.shape}"
            )

            # Store data for compatibility with existing code structure
            single_pollutant_data = {
                "X_train": X_train_seq,
                "y_train": y_train_single_pollutant,
                "X_val": X_val_seq,
                "y_val": y_val_single_pollutant,
                "X_test": X_test_seq,
                "y_test": y_test_single_pollutant,
                "target_scaler": target_scaler,
                "outlier_stats": {},
            }

            if not args.eval_only:
                # Training mode - build new model from scratch
                print("\n==================== Training Mode ====================")
                print(f"\n{'='*60}")
                ...
                # Initialize and train CNN+LSTM model for this pollutant
                print(f"Training CNN+LSTM model for {pollutant_name}...")
                model, history = train.train_single_pollutant_cnn_lstm_model(
                    X_train_seq,
                    y_train_single_pollutant,
                    X_val_seq,
                    y_val_single_pollutant,
                    pollutant_name=pollutant_name,
                    epochs=config["epochs"],
                    batch_size=config["batch_size"],
                    resume=not args.no_resume,
                    use_generator=False,
                    model_builder=get_cnn_lstm_model,
                )
                ...
                config_data = {
                    ...
                    "data_shapes": {
                        "sequential_train": str(X_train_seq.shape),
                        "target_train": str(y_train_single_pollutant.shape),
                    },
                    "outlier_removal": {
        ...
        for pollutant_name in pollutant_names:
            ...
            y_pred_test_scaled = model.predict(
                X_test_seq
            )  # Predict on the 3D test features

            # Ensure predictions are 1D for single-pollutant models
            if y_pred_test_scaled.ndim > 1:
                y_pred_test_scaled = y_pred_test_scaled.ravel()

            # Transform back to original scale
            # The target_scaler was fitted on all 3 targets, so we need to create a dummy array
            # with the right shape and only use the relevant column
            dummy_pred = np.zeros((len(y_pred_test_scaled), 3))
            dummy_pred[:, target_idx] = y_pred_test_scaled
            y_pred_test_orig = target_scaler.inverse_transform(dummy_pred)[
                :, target_idx
            ]

            # **CRITICAL:** Adjust coordinates for spatial plots
            # The coordinate arrays are longer than the sequence arrays by (lookback - 1).
            # We need to slice them to match the predictions.
            y_test_true_single_pollutant = y_test_seq[:, target_idx]
            lons_test_aligned = lons_test[lookback - 1 :]
            lats_test_aligned = lats_test[lookback - 1 :]

            print(
                f"Coordinate alignment: original coords {len(lons_test)}, aligned coords {len(lons_test_aligned)}, predictions {len(y_pred_test_orig)}"
            )

            # For validation predictions (if needed)
            y_pred_val_scaled = model.predict(X_val_seq)
            if y_pred_val_scaled.ndim > 1:
                y_pred_val_scaled = y_pred_val_scaled.ravel()
            dummy_pred_val = np.zeros((len(y_pred_val_scaled), 3))
            dummy_pred_val[:, target_idx] = y_pred_val_scaled
            y_pred_val_orig = target_scaler.inverse_transform(dummy_pred_val)[
                :, target_idx
            ]

            # Get raw targets - need to inverse transform the scaled targets
            y_val_raw_single = target_scaler.inverse_transform(y_val_seq)[:, target_idx]
            y_test_raw_single = target_scaler.inverse_transform(y_test_seq)[
                :, target_idx
            ]
            ...
            # Spatial concentration map using aligned coordinates
            if lons_test_aligned is not None and lats_test_aligned is not None:
                print(f"Generating spatial concentration map for {pollutant_name}...")

                # Use aligned coordinates and concentrations
                lons = lons_test_aligned
                lats = lats_test_aligned
                ...
                # Create a mask for valid (non-NaN) data across all arrays
                valid_mask = (
                ...
                # Clip predictions at zero to prevent negative concentrations
                y_pred_spatial = np.maximum(0, y_pred_spatial)

                # Now use these aligned arrays for your spatial plots
                ...
                evaluate.plot_truth_prediction_bias_maps(
                    lons=lons,  # Use aligned coordinates
                    lats=lats,  # Use aligned coordinates
                    y_true=y_true_spatial,
                    y_pred=y_pred_spatial,
                    pollutant_name=pollutant_name,
                    shapefile_path=shapefile_for_map,
                ...
                print(f"Coordinates not available for {pollutant_name} spatial map")
            ...
            if not args.skip_shap and SHAP_AVAILABLE:
                try:
                    ...
                    background_data = X_train_seq[
                        np.random.choice(X_train_seq.shape[0], 100, replace=False)
                    ]
                    test_data_subset = X_test_seq[
                        np.random.choice(X_test_seq.shape[0], 500, replace=False)
                    ]
